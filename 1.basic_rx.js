...
// data
const d1 = ['d1','d2','d3']
const d2 = Rx.of('d1','d2','d3')
const d3 = new Promise((resolve,reject)=>setTimeout(()=>resolve('d1'),100))

// stream
const s1 = Rx.from(d1)
const s2 = Rx.from(d2)
const s_concat = Rx.concat(s1,s2)
const ss1 = Rx.interval(1000).pipe(take(3))
const ss2 = Rx.interval(1000).pipe(tap(d=>console.log("스트림에는 영향을 주지않음")))
const ss_merge = Rx.merge(ss1,ss2)
